...
export const DrawerProvider: FC<PropsWithChildren<DrawerProviderProps>> = ({ 
  ...
  const isOpen = useCallback((drawerId: string) => {
  ...
  const navigateFromDrawer = useCallback((drawerId: string, screen: string, params?: any) => {
    if (navigationRef.current) {
...
export const useDrawer: UseDrawerType = (drawerId = "app") => {
  ...
  return {
    ...
    navigateFromDrawer: (screen: string, params?: any) => context.navigateFromDrawer(drawerId, screen, params),
